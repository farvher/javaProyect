...
public class Controlusuario extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
          ...
          if(!USER.getRol().equals("Inactivo") ){
          response.sendRedirect("nuevoregistro.jsp");
          }else{ response.sendRedirect("productos.jsp");}
           }else if (USER.getCedula() == 0){ response.sendRedirect("index.jsp?mensaje=Usuario incorrecto");}
           else{ response.sendRedirect("index.jsp?mensaje=Usuario incorrecto");}
